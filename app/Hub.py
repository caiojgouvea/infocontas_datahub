...
class Hub:
    ...
    def from_app_config(cls, *, app_config: AppConfig):
        ...
        return cls(
            ...
            download_uc=DownloadDataset(registry=registry, storage=storage, app_config=app_config,contract_service=contract,),)

    def execute_command(self, *, command: str, **k):
        if command == "export":
            return self.export_uc.execute(dataset=k["dataset"], version=k["version"], ano=k["ano"])
        if command == "download":
            return self.download_uc.execute(dataset=k["dataset"], version=k["version"], ano=k["ano"], tc=k["tc"])
        if command == "download-all-years":
            return self.download_uc.execute_all_years(dataset=k["dataset"], version=k["version"], tc=k["tc"])
