...
class PartitionSpec:
    ...
    def from_contract(contract) -> "PartitionSpec":
        ...
        spec = PartitionSpec(tuple(parsed))
        spec.validate_ctx_prefix_order()
        return spec

    def validate_ctx_prefix_order(self) -> None:
        found_dataframe = False

        for field in self.fields:
            if field.source_from == "dataframe":
                found_dataframe = True
                continue

            if found_dataframe and field.source_from == "ctx":
                raise ValueError(
                    "Particionamento inválido: campos 'ctx' devem vir antes dos campos 'dataframe'. "
                    "Exemplo válido: tc(ctx), ano(ctx), mes(dataframe)."
                )
    ...
    def ctx_scope_drop_last_n(self) -> int:
        ctx_prefix_count = 0
        ...
        if ctx_prefix_count == 0:
            raise ValueError("Contrato sem partição inicial vinda do contexto; não há escopo seguro para limpeza/publicação")

        return len(self.fields) - ctx_prefix_count

    def values_from_ctx(self, ctx, *, drop_last_n: int = 0) -> dict[str, object]:
        values: dict[str, object] = {}

        for field in self.get_fields(drop_last_n=drop_last_n):
            if field.source_from != "ctx":
                raise ValueError(
                    f"Partição '{field.name}' vem do DataFrame e não pode ser resolvida apenas pelo contexto"
                )
